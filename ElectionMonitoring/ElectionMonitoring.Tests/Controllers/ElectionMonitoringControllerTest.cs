...
namespace ElectionMonitoring.Tests.Controllers
{
    ...
    public class ElectionMonitoringControllerTest
    {
        ...
        private IEnumerable<Models.Region> regions;
        ...
        public void SetUp()
        {
            ...
            {
                new Models.Race{ RaceID = 1, RaceTypeID = 1, Year = 2013},
                new Models.Race{ RaceID = 2, RaceTypeID = 2, Year = 2013},
                new Models.Race{ RaceID = 3, RaceTypeID = 2, Year = 2013},
            };

            raceResults = new List<Models.AggregatedRaceResult>
            {
                new Models.AggregatedRaceResult{ CandidateID = 1, FirstName = "Abba", LastName= "Don", PartyAcronym = "AD", RaceID = 1, TotalVotes = 2554544, RegionCode = "AB", PartyName = "Alliance for Democracy"},
                new Models.AggregatedRaceResult{ CandidateID = 2, FirstName = "Phil", LastName= "Peter", PartyAcronym = "PDP", RaceID = 1, TotalVotes = 36345, RegionCode = "AB", PartyName = "People's Democratic Party"},
            };
            regions = new List<Models.Region>
            {
                new Region { RegionID = 1, ParentRegionID = 0, RegionCode = "AB", Name = "Abia", TopLevel = true},
                new Region { RegionID = 38, ParentRegionID = 1, RegionCode = "AB.AS", Name = "Aba South", TopLevel = false},
                new Region { RegionID = 39, ParentRegionID = 1, RegionCode = "AB.AN", Name = "Aba North", TopLevel = false},
                new Region { RegionID = 37, ParentRegionID = 0, RegionCode = "ZA", Name = "Zamfara", TopLevel = true},
                new Region { RegionID = 700, ParentRegionID = 37, RegionCode = "ZA.MA", Name = "Malif", TopLevel = false},
            };
            ...
            mockRegionRepo.Setup(r => r.GetRegions(It.IsAny<int>())).Returns(regions.Where(re => re.ParentRegionID == 1).ToList());
            mockRegionRepo.Setup(r => r.GetRegions()).Returns(regions);            

            mockRaceResultRepo = new Mock<IRaceResultService>();
            mockRaceResultRepo.Setup(r => r.GetAggregatedRaceResults(It.IsAny<int>(), It.IsAny<string>())).Returns(raceResults);
        ...
        public ElectionMonitoringController SetupControllerForTests(Mock<IRaceResultService> mockRaceResultRepo,
            Mock<IRegionRepository> mockRegionRepo, Mock<IRaceRepository> mockRaceRepo)
        {
            
            var controller = new ElectionMonitoringController(mockRaceResultRepo.Object, mockRegionRepo.Object, mockRaceRepo.Object );
